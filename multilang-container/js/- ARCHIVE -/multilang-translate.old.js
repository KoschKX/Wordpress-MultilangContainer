...
(() => {
    ...
    if (window.multilangLangBar && window.multilangLangBar.translationMethod === 'server') {
        console.log('⚠️ Early return: translationMethod is server');
        return;
    }
    
    console.log('✅ Passed translationMethod check');
    ...
    function initializeImmediately() {
        console.log('🔧 initializeImmediately() called');
    ...
    function runTranslations() {
        console.log('▶️ runTranslations() called');
        ...
        if (!currentLangTranslations && !defaultLangTranslations) {
            console.log('⚠️ No translations found, exiting runTranslations()');
            return;
        }
        ...
        console.log('🔍 Sections found:', Object.keys(structureData));
        ...
        Object.keys(structureData).forEach(function(sectionName) {
            ...
            if (sectionConfig && typeof sectionConfig === 'object' && sectionConfig['_selectors']) {
                ...
                console.log('  Method:', sectionMethod);
                if (sectionMethod === 'javascript') {
                    console.log('  ✅ Processing', sectionName, 'with JavaScript');
                    ...
                    if (Array.isArray(selectors) && selectors.length > 0) {
                        ...
                        if (Object.keys(sectionTranslations).length > 0) {
                            console.log('  📝 Section translations prepared for', sectionName);
                            console.log('  🎯 Selectors:', selectors);
                            
                            // Debug: Check if selectors match any elements
                            selectors.forEach(function(selector) {
                                var matchingElements = document.querySelectorAll(selector);
                                console.log('    Selector "' + selector + '" matched', matchingElements.length, 'elements');
                            });

                            translateLang(sectionTranslations, selectors);
                        } else {
                            console.log('  ⚠️ No translations found for', sectionName);
                        }
                    }
                }
            }
    ...
    function updateLanguageDisplay(newLang) {
        ...
        for (let i = 0; i < allElements.length; i++) {
            ...
            if (element.classList.contains('translate') || element.closest('.translate')) {
                console.log('  🎯 updateLanguage - element:', element, 'elementLang:', elementLang, 'newLang:', newLang);
                if (elementLang === newLang) {
                    // Show this element
                    console.log('    ✅ Showing (matches current lang)');
                    element.style.display = '';
                } else {
                    // Hide other language elements
                    console.log('    ❌ Hiding (different lang)');
        ...
        for (let i = 0; i < multilingualWrappers.length; i++) {
            ...
            // Check if wrapper has mixed content (translation spans + plain text nodes)
            // For partial translations, we don't need a fallback
            ...
            // If no element is visible for the current language AND no plain text content, show fallback
            if ((!currentLangElement || visibleElements.length === 0) && !hasPlainText) {
                ...
                if (originalText) {
                    ...
                    if (!fallbackSpan) {
                        ...
                        wrapper.appendChild(fallbackSpan);
                    }
                    if (defaultText) {
                ...
                // Hide fallback if we have translations for current language OR plain text content
                const fallbackSpan = wrapper.querySelector('.translate-fallback');
                if (fallbackSpan) {
                    fallbackSpan.style.display = 'none';
                }
    ...
    function translateLang(sectionTranslations, targets) {
        console.log('🌍 translateLang() called');
        console.log('  targets:', targets);
        console.log('  sectionTranslations:', sectionTranslations);
        
        targets = targets || ['body'];
        if (!sectionTranslations) {
            console.log('  ⚠️ No sectionTranslations, exiting');
            return;
        }
        ...
        console.log('  📦 Elements found before filtering:', elements.length);
        ...
        console.log('  ✅ Elements after filtering:', elements.length);
        
        if (elements.length === 0) {
            console.log('  ⚠️ No elements to process!');
            return;
        }
        ...
        function translateText(text, sectionTranslations) {
            ...
            tokens.forEach(function(token) {
                ...
                if (Object.keys(tokenTranslations).length > 0) {
                    ...
                    // Get the original token (without punctuation) for data-original-text
                    var originalToken = token.replace(/^[^\p{L}\p{N}]+|[^\p{L}\p{N}]+$/gu, '');
                    var defaultTokenText = tokenTranslations[defaultLang] || originalToken;
                    ...
                    availableLanguages.forEach(function(lang) {
                        if (tokenTranslations[lang]) {
                            ...
                            if (hideFilterEnabled) {
                                ...
                                tokenSpans += '<span class="translate lang-' + lang + '" data-translation="' + encoded_content + '" data-original-text="' + originalToken + '" data-default-text="' + defaultTokenText + '">' + tokenTranslations[lang] + '</span>';
                            } else {
                                // When hide filter is not active, use display styles  
                                var encoded_content = encodeForDataAttr(tokenTranslations[lang]);
                                tokenSpans += '<span class="translate lang-' + lang + '" data-translation="' + encoded_content + '" data-original-text="' + originalToken + '" data-default-text="' + defaultTokenText + '" style="display: ' + display + ';">' + tokenTranslations[lang] + '</span>';
                            }
                        }
                    });
        ...
        function wrapTextNodes(element) {
            console.log('📄 wrapTextNodes() called for element:', element.tagName, element.className);
            
            const skipTags = ['SCRIPT', 'STYLE', 'CODE', 'PRE'];
            if (skipTags.includes(element.tagName)) {
                console.log('  ⏭️ Skipping tag:', element.tagName);
                return;
            }
            if (excludedSelectors.some(sel => element.closest(sel))) {
                console.log('  ⏭️ Excluded by selector');
            ...
            if (
                ...
                console.log('  ⏭️ Contains code class');
                return;
            }
            ...
            if (element.closest('.multilang-wrapper') || element.closest('.translate')) {
                console.log('  ⏭️ Already inside wrapper/translate');
                return;
            }
            
            console.log('  ✅ Processing child nodes of', element.tagName);

            element.childNodes.forEach(function(node) {
                if (node.nodeType === Node.TEXT_NODE) {
                    console.log('    📝 Found TEXT_NODE:', node.textContent);
                    var originalText = node.textContent.trim();
                    if (!originalText) {
                        console.log('      ⏭️ Empty text node, skipping');
                        return; // Skip empty text nodes
                    }
                    
                    console.log('      🔄 Calling translateText for:', originalText);
                    var translated = translateText(originalText, sectionTranslations);
                    console.log('      📤 Translation result:', translated);
                    console.log('      🔍 Changed?', translated !== originalText);
                    
                    if (translated !== originalText) {
                        console.log('      🔨 Creating wrapper for:', originalText);
                        ...
                        if (sectionTranslations && sectionTranslations[defaultLang]) {
                            // Try direct key using the wrapper's original text
                        ...
                        wrapper.querySelectorAll('.translate[class*="lang-"]').forEach(function(span) {
                            ...
                            // For data-default-text, use the span's data-original-text (which might be a token)
                            var spanOriginalText = span.getAttribute('data-original-text') || originalText;
                            // For data-default-text, use the span's data-original-text (which might be a token)
                            var spanOriginalText = span.getAttribute('data-original-text') || originalText;
                            ...
                            if (isHideFilterActive) {
                                // Add data-translation attribute with matching PHP encoding
                                // DON'T clear content - token-based translations already have correct content in each span
                        ...
                        console.log('      🔄 Replacing node with wrapper');
        ...
        console.log('✨ translateLang() completed');
    }
