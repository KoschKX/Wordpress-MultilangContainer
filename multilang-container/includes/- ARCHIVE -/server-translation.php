...
function multilang_wrap_text_nodes_selective($body, $current_lang_translations, $default_lang_translations, $current_lang, $default_lang) {
    ...
    foreach ($sections_cache as $section => $section_xpaths) {
        ...
        $uses_body_selector = false;
        if (isset($structure_data_cache[$section]['_selectors'])) {
            $section_selectors = $structure_data_cache[$section]['_selectors'];
            if (is_array($section_selectors) && in_array('body', $section_selectors)) {
                $uses_body_selector = true;
            }
        }
        
        // If using body selector, use ALL translations; otherwise use section-specific
        if ($uses_body_selector) {
            $section_current_lang = $current_lang_translations;
            $section_default_lang = $default_lang_translations;
        } else {
            // Filter translations to ONLY this section's category
        ...
        foreach ($section_xpaths as $sel) {
            ...
            foreach ($matching_elements as $element) {
                ...
                $replacements_made += multilang_wrap_text_nodes($element, $section_current_lang, $section_default_lang, $current_lang, $default_lang);
            }
        }
    }
