 ...
 * 
 * IMPORTANT: Now respects section selectors from structure.json
 * - Only processes sections configured with _method: "server"
 * - Leaves client-side sections untouched for JavaScript to handle
 * - Uses the same selector logic as server-translation.php
 * 
 * Simple logic: Remove content from spans that are NOT the current language
 * but ONLY within server-side translation sections
 */
 ...
 * Now respects section selectors from structure.json
 */
function multilang_hide_non_current_language($content) {
    ...
    $structure_data = null;
    $server_sections_selectors = array();
        ...
        if ($structure_data && is_array($structure_data)) {
            foreach ($structure_data as $section => $config) {
                ...
                if (is_array($selectors)) {
                    $server_sections_selectors = array_merge($server_sections_selectors, $selectors);
                }
            }
        }
    }
    
    // If no server sections found, return unchanged
    if (empty($server_sections_selectors)) {
        return $content;
    }
    ...
    foreach ($server_sections_selectors as $css_selector) {
        $xpath_selector = multilang_hide_filter_css_to_xpath($css_selector);
        if (!$xpath_selector) continue;
        
        // Find containers matching this selector
        $containers = $xpath->query($xpath_selector);
        
        foreach ($containers as $container) {
            // Find translation elements within this container
            $translation_elements = $xpath->query('.//*[contains(@class, "translate") and contains(@class, "lang-")]', $container);
 ...
 * Used by hide filter to match section selectors
 */
function multilang_hide_filter_css_to_xpath($selector) {
